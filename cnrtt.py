...
CONFIG_FILE = "rtt_history.json"

class RTTViewerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("第三方 RTT Viewer (支持中文)")
        ...
        self.output_text = scrolledtext.ScrolledText(mid_frame, wrap=tk.WORD, font=("Consolas", 10), bg="#f0f0f0")
        self.output_text.pack(fill=tk.BOTH, expand=True)
        
        # 拦截键盘输入，只允许复制和全选，防止误改日志
    ...
    def connect(self):
        try:
            ...
            # 移除 rtt_is_ok 检查，改为延时等待底层初始化，兼容所有 pylink 库版本
            time.sleep(0.2)
